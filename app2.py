...
retriever = db.as_retriever(
    search_kwargs={"k": 2}  # Limite quantos documentos retorna para não estourar tokens
)
...
Priorize as informações desses documentos para fornecer respostas precisas, concisas e baseadas em fatos. Se a respostas for no formato SIC, 
...
substiuindo conforme input que receber os trechos sei_number, sic_summary, sic_extract e context. Caso contrário, responda de forma geral.

{% if request_type == "SIC" %}
Prezado(a) Senhor(a),
1. Faço referência ao Pedido de Informação (SEI {{ sei_number }}), que em resumo ({{ sic_summary }}), e no extrato o requerente assim se manifesta: "{{ sic_extract }}"
...
3. Nesse sentido, especificamente em relação à demanda do requerente, {{ context }}

A API de Compras disponibiliza ampla quantidade de recursos em dados abertos relacionados às compras 
...
3.1. Módulo 1: Catálogo de Materiais (CATMAT): Permite a consulta de itens de materiais, com
dados detalhados como quantidade, preço unitário, fabricante e marca.
3.2. Módulo 2: Catálogo de Serviços (CATSER): Permite consultar os serviços com informações
detalhadas.
3.3. Módulo 3: Preços Praticados: Permite consultar os preços praticados nas compras públicas,
utilizando os códigos dos itens do CATMAT e CATSER.
3.4. Módulo 4: Planejamento e Gerenciamento das Contratações (PGC): Oferece dados sobre o
...
3.5. Módulo 5: Unidades Administrativas de Serviços Gerais (UASG): Traz informações
...
3.6. Módulo 6: Legado: Permite a consulta das compras e contratações realizadas sob a vigência
...
3.7. Módulo 7: Contratações (Lei nº 14.133/2021): Este módulo é focado em compras e
...
3.8. Módulo 8: ARP - Ata de Registro de Preço: Possibilita a consulta de Atas de Registro de Preço
...
3.9. Módulo 9: Contratos: Permite a consulta de contratos e seus itens, e traz informações
...
3.10. Esses módulos permitem o acesso detalhado às informações de homologações e licitações,
...
4. Ademais, no intuito de contribuir com a busca de informações em dados abertos sobre
Compras Públicas, recomendamos outros Painéis e Portais:
4.1. Painel de Compras Governamentais: Recomendamos o uso do Painel de Compras, que
...
4.2. Consulta Detalhada no Portal de Compras Governamentais: Para acessar diretamente os
...
4.3. Portal Nacional de Contratações Públicas (PNCP): O PNCP oferece consultas centralizadas
...
4.4. Painel PNCP em números: painel interativo que mostra o ecossistema de compras públicas
...
5. Cabe ainda informar que é possível realizar buscas na transparência ativa do Catálogo do
...
5.1. Catálogo: https://catalogo.compras.gov.br/cnbs-web/busca
5.2. Pesquisa de Preços: https://www.gov.br/compras/pt-br/sistemas/conheca-o-compras/pesquisa-de-precos

6. Por fim, informa-se que é possível encontrar dados sobre contratos por meio do seguinte
caminho:
6.1. Contratos.gov.br: repositório contendo informações detalhadas sobre os empenhos e
...
7. O Governo Federal está comprometido em promover a transparência dos gastos públicos,
